...
public class TwitchStream
{
    ...
    public static final String API_URL = "https://api.twitch.tv/kraken/streams/";
...
//        }
    ...
    /**
     * Creates a TwitchStream object from https://api.twitch.tv/teocci/channels/channelname
     *
     * @param channelName
     * @return
     * @throws IOException
    ...
    {
        String data = Network.getJSON(TwitchUserInfo.getTwitchChannelStreamTwitchStreamVideosURL(channelName));
        if (data == null) return null;
        JsonObject p = new Gson().fromJson(data, JsonObject.class);
        if (p == null) return null;
        System.out.println(p);
        if (p.has("stream") && !(p.get("stream") instanceof JsonNull)) {
            TwitchStream stream = new Gson().fromJson(p, TwitchStream.class);
